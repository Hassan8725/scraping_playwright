...
with sync_playwright() as p:
    ...
    lang_selector_1 = ".dl-profile-card-content > .dl-profile-row-content > div:nth-child(1)"
    ...
    lang_selector_2 = ".dl-profile-card-content > .dl-profile-row-content > div:nth-child(1)"
        ...
        if len(all_link_elements) >= 1 and len(all_name_elements) >= 1 and len(all_info_elements) >= 1:
            for link_elem, name_elem, info_elem in zip(all_link_elements, all_name_elements, all_info_elements):
                ...
                if href and name and info:
                    ...
                    all_services_elements = doctor_page.query_selector_all(services_selector_1)

                    all_bio_elements = doctor_page.query_selector(profile_bio_selector_1)
                    lang_element = doctor_page.query_selector(lang_selector_1)
                    website_element = doctor_page.query_selector(website_selector_1)

                    if len(all_services_elements) != 0: # or len(all_bio_elements) != 0 or len(lang_element) != 0 or len(website_element) != 0:

                        all_services_elements = doctor_page.query_selector_all(services_selector_2)

                        all_bio_elements = doctor_page.query_selector(profile_bio_selector_2)
                        lang_element = doctor_page.query_selector(lang_selector_2)
                        website_element = doctor_page.query_selector(website_selector_2)

                    profile_bio = all_bio_elements.evaluate('(element) => element.textContent')  

                    if lang_element:
                        languages = lang_element.evaluate('(element) => element.textContent')
                    else:
                        languages = ""
                    ...
                    for elem in all_services_elements:
                        services = ' | '.join(sub_div.text_content().strip() for sub_div in elem.query_selector_all('div'))  
    ...
    with open('doctors_data7.json', 'w', encoding='utf-8') as file:
